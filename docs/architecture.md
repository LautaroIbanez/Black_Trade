...
- Signal computation: The processor asynchronously triggers `SignalComputationService.compute_and_store()` with impacted timeframes. The service loads recent OHLCV from `MarketDataService`, runs `RecommendationService.generate_recommendation`, and persists a JSON snapshot per timeframe in `live_recommendations` via `LiveResultsRepository`.
- Availability: Frontend queries live endpoints (`/recommendation`, `/api/metrics`, `/api/risk/status`) and subscribes to WebSocket broadcasts for risk/alerts/orders. Backend has no dependency on CSV.
...
5. t0+≤2.5s: Frontend sees updated recommendation on next poll; risk/events stream over WebSocket immediately
